...
public interface IReviewRepository:IRepository<Review,int>
{
    ...
    Task<List<Review>> GetPagedByMovieIdAsync(int movieId,int page,int pageSize,CancellationToken cancellationToken);
    Task<int> GetCountByUserIdAsync(string userId, CancellationToken cancellationToken);
    Task<List<Review>> GetPagedByUserIdAsync(string userId,int page,int pageSize, CancellationToken cancellationToken);
